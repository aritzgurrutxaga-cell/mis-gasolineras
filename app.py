...
# SI YA TENEMOS UN MUNICIPIO GUARDADO, SALTAMOS DIRECTAMENTE A RESULTADOS
if st.session_state.municipio_guardado and not st.session_state.solicitar_gps:
    pass # Continúa a la sección de Resultados

# PANTALLA 1: INICIO (Solo si no hay municipio y no se ha pedido GPS)
elif not (estado_permiso == "granted" or st.session_state.municipio_guardado) and not st.session_state.solicitar_gps:
...
# GPS (Solo si se ha solicitado o ya tenía permiso)
...
# PANTALLA 2: SELECCIÓN MANUAL (Solo si el GPS falló y no hay municipio guardado)
...
if lat_gps and not st.session_state.override_manual:
    ...
    # Buscamos la fila para obtener coordenadas de referencia del municipio
    filas_muni = df[df["Municipio"] == muni_ref]
    if not filas_muni.empty:
        fila = filas_muni.iloc[0]
        lat_ref, lon_ref = fila["lat_num"], fila["lon_num"]
    else:
        # Fallback por si el municipio guardado ya no existe en los datos
        st.session_state.municipio_guardado = None; st.rerun()
